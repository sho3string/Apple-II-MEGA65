...
entity keyboard_adapter is
    port (
        keyboard_n       : in  std_logic_vector(79 downto 0);
        kb_key_pressed_n : in  std_logic;

        -- Keyboard mapping:
        -- '0' = Apple II positional/default mapping
        -- '1' = MEGA65 glyph/symbol mapping
...
architecture Behavioral of keyboard_adapter is
    ...
    ---------------------------------------------------------------------------
    -- Private MEGA65 glyph pseudo scan codes
    --
    -- These are NOT real PS/2 codes.
    --
    -- keyboard_apple recognises these and maps them onto the appropriate
    -- Apple II keyboard matrix location while forcing Shift on/off where
    -- required.
    ---------------------------------------------------------------------------
    ...
    -- No character output.
    -- keyboard_apple has no junction mapping for FA, therefore it becomes FF
    -- and is ignored.
    constant m65_code_noop        : std_logic_vector(7 downto 0) := x"FA";

    constant m65_code_semicolon   : std_logic_vector(7 downto 0) := x"FB"; -- ; forced unshift
    ...
    ---------------------------------------------------------------------------
    -- Physical MEGA65 keys represented by each mapping entry.
    ---------------------------------------------------------------------------
    ...
    constant m65_keys : integer_vector := (
        ...
        -- MEGA65/C64-style up-arrow key.
        -- Repurposed as Apple backslash because MEGA65 has no dedicated \ key.
        60 => m65_arrow_up,
    ...
    ---------------------------------------------------------------------------
    -- Original/default Apple II positional mapping
    ---------------------------------------------------------------------------
    ...
    constant ps2_codes_apple : slv8_array := (
        ...
        -- Original positional Apple layout
        ...
        -- Convenience mapping for dedicated £/# key.
        63 => m65_code_hash,
        
        64 => m65_code_noop -- MEGA65 ← unused in original Apple layout
    );


    ---------------------------------------------------------------------------
    -- MEGA65 glyph-oriented mapping
    --
    -- Dedicated MEGA65 symbol keys generate the glyph printed on the keycap
    -- rather than preserving Apple keyboard physical position.
    ---------------------------------------------------------------------------
    ...
    constant ps2_codes_mega65 : slv8_array := (
        ...
        64 => apple_keycode_backtick  -- ← -> ` / Shift+← -> ~
    );
...
begin
    ...
    ---------------------------------------------------------------------------
    -- MEGA65 key -> Apple II PS/2-style event converter
    --
    -- ps2_key(10)       event toggle
    -- ps2_key(9)        1 = key press, 0 = key release
    -- ps2_key(8)        extended scan code
    -- ps2_key(7 downto 0) scan/pseudo code
    ---------------------------------------------------------------------------

    keyboard_map_proc : process(CLK_14M)
        ...
        if rising_edge(CLK_14M) then
            ...
            if reset = '1' then
                ...
                loop
                    ...
                    -----------------------------------------------------------
                    -- Detect physical MEGA65 key transition
                    -----------------------------------------------------------
                    if keyboard_n(m65_keys(current_key_index)) /=
                       prev_keyboard_n(m65_keys(current_key_index))
                    then

                        prev_keyboard_n(m65_keys(current_key_index))
                            <= keyboard_n(m65_keys(current_key_index));
                        -------------------------------------------------------
                        ...
                        -------------------------------------------------------
                        ps2_key(9)
                            <= not keyboard_n(m65_keys(current_key_index));
                        -------------------------------------------------------
                        -- Extended PS/2 codes
                        -------------------------------------------------------
                        ...
                        -------------------------------------------------------
                        -- Only issue one key event in this clock cycle.
                        -------------------------------------------------------
                        exit;
                    end if;
                end loop;
